...
class RegexEngineTest  {
  ...
  private def assertGroupsEquals(m: Matcher, expectedGroups: String*): Matcher = {
    ...
    if (groupCount == 1) // nicer error messages for a single group
  ...
  private def assertSyntaxError(pattern: String, flags: Int, desc: String, index: Int): Unit = {
    val th = expectThrows(classOf[PatternSyntaxException], compile(pattern, flags))
    if (!executingInJVM) {
  ...
  @Test def predefinedCharacterClasses(): Unit = {
    ...
    // Interestingly, \p{Lower} with CaseInsensitive is still case-sensitive
    val lowerCI = compile("\\p{Lower}", CaseInsensitive)
    assertMatches(lowerCI, "a")
    assertMatches(lowerCI, "f")
    assertMatches(lowerCI, "z")
    assertNotMatches(lowerCI, "A")
    assertNotMatches(lowerCI, "G")
  ...
  @Test def predefinedUnicodeCharacterClasses(): Unit = {
    ...
    /* Interestingly, \p{Lower} with CaseInsensitive is still case-sensitive
     * according to the JVM. However, JavaScript's RegExp does not behave that
     * way, so it is not possible for us to support that.
     */
