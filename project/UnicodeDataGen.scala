...
object UnicodeDataGen {
  ...
  private final val DiffBits = 13
  private final val DiffShift = 32 - DiffBits
  private final val TypeBits = 5 // types are in [0, 30]
  private final val PropBits = DiffShift - TypeBits - 1 // 1 for the alternatingTypes bit

  /** Number of "large" diffs that we tolerate. */
  private final val LargeDiffsCount = 16

  /** The threshold above which a diff is considered large. */
  private final val LargeDiffThreshold = (1 << DiffBits) - LargeDiffsCount
  ...
  private final class BooleanProp(val name: String, val computed: Boolean,
      val testFun: Int => Boolean) {

    override def toString(): String = name
  }

  /** Makes a test function through a reflective call on `jl.Character`.
   *
   *  Use this for methods added to `Character` after JDK 8. Otherwise, the
   *  build cannot compile on earlier versions.
   */
  private def testFunWithReflectiveCall(methodName: String): Int => Boolean = {
    val m = classOf[Character].getDeclaredMethod(methodName, classOf[Int])

    { (cp: Int) => m.invoke(null, Integer.valueOf(cp)).asInstanceOf[Boolean] }
  }
  ...
  private def isOtherAlphabetic(cp: Int): Boolean =
    isAlphabetic(cp) && !isLetter(cp) && getType(cp) != Character.LETTER_NUMBER
  ...
  private val AllBooleanProps: List[BooleanProp] = List(
    ...
    new BooleanProp("Ideographic", false, isIdeographic(_)),
    new BooleanProp("OtherIDStart", false, isOtherIDStart(_)),
    new BooleanProp("OtherIDPart", false, isOtherIDPart(_)),
    new BooleanProp("Mirrored", false, isMirrored(_)),
    new BooleanProp("Emoji", false, testFunWithReflectiveCall("isEmoji")),
    new BooleanProp("EmojiPresentation", false, testFunWithReflectiveCall("isEmojiPresentation")),
    new BooleanProp("EmojiModifier", false, testFunWithReflectiveCall("isEmojiModifier")),
    new BooleanProp("EmojiModifierBase", false, testFunWithReflectiveCall("isEmojiModifierBase")),
    new BooleanProp("EmojiComponent", false, testFunWithReflectiveCall("isEmojiComponent")),
    new BooleanProp("ExtendedPictographic", false, testFunWithReflectiveCall("isExtendedPictographic")),
    new BooleanProp("NonWhitespaceISOControl", true, isNonWhitespaceISOControl(_)),
    new BooleanProp("Whitespace", true, isWhitespace(_)),
    //new BooleanProp("CombineClassIsAbove", cp => computeCombiningClass(cp) == CombiningClassIsAbove),
    //new BooleanProp("CombineClassIsOther", cp => computeCombiningClass(cp) == CombiningClassIsOther)
  )

  assert(AllBooleanProps.count(!_.computed) <= PropBits, "Too many boolean properties")
  ...
  private def generateUnicodeData(): Unit = {
    ...
    val constants = List(
      constantDef("DiffBits", DiffBits),
      constantDef("PropBits", PropBits),
      constantDef("LargeDiffThreshold", LargeDiffThreshold),
    ...
    val largeDiffsCPs = mutable.ArrayBuffer.empty[Int]

    var prevCP = 0
    val intDataRanges = dataRanges.map { e =>
      val diff = e.firstCP - prevCP
      val diffBits = if (diff >= LargeDiffThreshold) {
        largeDiffsCPs += e.firstCP
        LargeDiffThreshold + (largeDiffsCPs.size - 1)
      } else {
        diff
      }
      prevCP = e.firstCP
      val truncatedData = dataToInt(e.data, propFlags) & ((1 << DiffShift) - 1)
      (diffBits << DiffShift) | truncatedData
    }

    val largeDiffMappings = for ((cp, index) <- largeDiffsCPs.toList.zipWithIndex) yield {
      s"case $index => 0x${cp.toHexString}"
    }
    ...
    // Check that we do not violate assumptions

    if (largeDiffsCPs.size > LargeDiffsCount) {
      throw new MessageOnlyException(
          largeDiffsCPs.map(_.toHexString).mkString("Too many large diffs: ", ", ", ""))
    }
    ...
    patchFile("javalib/src/main/scala/java/lang/UnicodeData.scala")(
      ...
      "large-diff-mappings" -> Patch.Lines(largeDiffMappings)
    )
  }
  ...
  private def computeDataRangesInitial(firstRangeStart: Int,
    ...
    for (cp <- (firstRangeStart + 1) to lastCP) {
  ...
  private def computePropFlags(dataRanges: Array[Entry],
    ...
    // Sort by frequencies, but keep non-computed properties first
    val sortedProps = frequencies.toList.sortBy(f => (f._1.computed, -f._2, f._1.name))
    val allocs =
      for ((propFreq, index) <- sortedProps.zipWithIndex)
        yield propFreq._1 -> (1 << (TypeBits + 1 + index)) // 1 for the alternatingTypes bit
    allocs.toMap
  }
  ...
  private val AllBooleanPropsForTests: List[BooleanProp] = List(
    ...
    new BooleanProp("isEmoji", false, testFunWithReflectiveCall("isEmoji")),
    new BooleanProp("isEmojiPresentation", false, testFunWithReflectiveCall("isEmojiPresentation")),
    new BooleanProp("isEmojiModifier", false, testFunWithReflectiveCall("isEmojiModifier")),
    new BooleanProp("isEmojiModifierBase", false, testFunWithReflectiveCall("isEmojiModifierBase")),
    new BooleanProp("isEmojiComponent", false, testFunWithReflectiveCall("isEmojiComponent")),
    new BooleanProp("isExtendedPictographic", false, testFunWithReflectiveCall("isExtendedPictographic")),
  ...
  private def generateUnicodeDataTest(): Unit = {
    ...
    while (i != entries.length) {
      val entry = entries(i)
      val lastCP =
        if (i == entries.length - 1) MAX_CODE_POINT + 1
    ...
    patchFile("test-suite/shared/src/test/require-jdk21/org/scalajs/testsuite/javalib/lang/UnicodeDataTest.scala")(
      "constants" -> Patch.Lines(constants),
