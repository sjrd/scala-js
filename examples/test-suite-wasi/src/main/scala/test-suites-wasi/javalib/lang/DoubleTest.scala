...
class DoubleTest {
  ...
  def toStringWithIntegerValuesWhenAnInteger(): Unit = {
    if (executingInJVM) {
  ...
  def parseDoubleInvalidThrows(): Unit = {
    for (padding <- List("", "  ", (0 to 0x20).map(x => x.toChar).mkString)) {
      def pad(s: String): String = padding + s + padding
  ...
  def compareToConvertedFromInt_Issue3085(): Unit = {
    @noinline
    def foo(x: Int): Unit =
      bar(x.toDouble)
