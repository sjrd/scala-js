...
class CharacterUnicodeBlockTest {
  ...
  def forNameNormalizations(): Unit = {
  ...
  def ofIntOutOfRangeThrowsIllegalArgumentException(): Unit = {
    assertThrows(classOf[IllegalArgumentException], UnicodeBlock.of(Character.MAX_CODE_POINT + 1))
  }

  def forNameNotFoundThrowsIllegalArgumentException(): Unit = {
