...
class NameEncodingTest {
  ...
  def namesThatAreJSReservedWords_Issue153(): Unit = {
  ...
  def namesStartingWithDigit_Issue153(): Unit = {
    // scalastyle:off class.name
  ...
  def javaIdentPartNotJSIdentPart(): Unit = {
  ...
  def localEvalOrArguments_Issue743(): Unit = {
    val eval = 5
    assertEquals(5, eval)
    val arguments = "hello"
