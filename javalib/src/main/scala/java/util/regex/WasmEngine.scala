...
private[regex] object WasmEngine extends Engine {
   ...
   */
  private final class Captures(ranges: Array[Int]) {
    def this(n: Int) = {
  ...
  final class WasmRegExp(root: Matcher, groupNodeMap: Array[Matcher],
   ...
   */
  private sealed abstract class MatchResult
  ...
  private object Failure extends MatchResult
   ...
   */
  @FunctionalInterface
  private trait MatchContinuation {
   ...
   */
  private final class RepeatMatcher(m: Matcher, min: Int, max: Int,
      ...
      if (max == 0) {
        ...
        val d: MatchContinuation = { y =>
          if (min == 0 && y.endIndex == x.endIndex) {
            ...
            val max2 = if (max == Int.MaxValue) Int.MaxValue else max - 1
            loop(min2, max2, y, c)
          }
        }
   ...
   */
  private final class AtomicMatcher(m: Matcher) extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
   ...
   */
  private final class MatchAlternatives(ms: Array[Matcher]) extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
   ...
   */
  private object CaretAssertion extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
      if (x.endIndex == 0)
        c(x)
      else
   ...
   *  @see [[https://262.ecma-international.org/15.0/index.html#sec-compileassertion]]
   */
  private object DollarAssertion extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
   ...
   *  @see [[https://262.ecma-international.org/15.0/index.html#sec-compileassertion]]
   */
  private final class WordBoundaryAssertion(negated: Boolean) extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
  ...
  private abstract class AnyLiteralMatcher extends Matcher

  /** A forward literal matcher. */
  private final class LiteralMatcher(val literal: String) extends AnyLiteralMatcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
      import x._
  ...
  private final class BackwardLiteralMatcher(val literal: String) extends AnyLiteralMatcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
      import x._
   ...
   */
  private object ForwardDotMatcher extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
      import x._
   ...
   *  @see [[https://262.ecma-international.org/15.0/index.html#sec-compileatom]]
   */
  private object BackwardDotMatcher extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
      import x._
      if (endIndex > 0)
        c(MatchState(input, input.offsetByCodePoints(endIndex, -1), captures))
   ...
   *  @see [[https://262.ecma-international.org/15.0/index.html#sec-compileatom]]
   */
  private final class ForwardCharacterSetMatcher(
   ...
   *  @see [[https://262.ecma-international.org/15.0/index.html#sec-compileatom]]
   */
  private final class BackwardCharacterSetMatcher(
      charSet: CharSet, invert: Boolean, unicodeIgnoreCase: Boolean)
      extends Matcher {
  ...
  private final class NonCapturingGroupMatcher(m: Matcher) extends Matcher {
   ...
   */
  private final class BackReferenceMatcher(groupNumber: Int, forward: Boolean) extends Matcher {
    def apply(x: MatchState, c: MatchContinuation): MatchResult = {
      val input = x.input
      val cap = x.captures
  ...
  private final class Parser(pattern0: String, unicodeIgnoreCase: Boolean) {
    ...
    private[this] var forward: Boolean = true
      ...
      while (true) {
        ...
        if (baseNode ne null) { // null if we just completed an alternative
          ...
          (pattern.charAt(pIndex): @switch) match {
            case '+' =>
              pIndex += 1
              finishRepeater(min = 1, max = Int.MaxValue)

            case '*' =>
              pIndex += 1
              finishRepeater(min = 0, max = Int.MaxValue)
    ...
    private def parseCharacterClass(): CharSet = {
      ...
      while (pattern.charAt(pIndex) != ']') {
        ...
        val cpStart = if (cp1 == '\\') {
          val cpStart = pattern.charAt(pIndex)
          pIndex += 1
          if (cpStart == 'p' || cpStart == 'P') {
    ...
    private def parseUnicodePropertyWithBraces(): CharSet = {
      LinkingInfo.linkTimeIf(LinkingInfo.esVersion >= LinkingInfo.ESVersion.ES2018) {
        pIndex += 1 // '{'
        val startIndex = pIndex
        while (pattern.charAt(pIndex) != '}')
          pIndex += 1
        ...
        // Don't link the giant tables in UnicodeProperties if we cannot use them
        throw new AssertionError(
            "PatternCompiler should have rejected Unicode properties")
      }
    }
  }
}
