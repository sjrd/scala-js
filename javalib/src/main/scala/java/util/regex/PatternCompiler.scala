...
private[regex] object PatternCompiler {
  ...
  private final class CharacterClassState(asciiCaseInsensitive: Boolean) {
    ...
    def finishSegment(): Unit = {
      if (!isSegmentNegated) {
        // Positive alternative -> directly compile as a normal alternative
        if (thisSegment != "")
          addAlternative("[" + thisSegment + "]")
      } else {
        // Negative alternative

        // If there are existing alternatives, wrap them in a negative look-ahead
        if (thisConjunct != "")
          thisConjunct = "(?!" + thisConjunct + ")"

        // Then add the negated character class for this segment
        thisConjunct += codePointNotAmong(thisSegment)
      }

      thisSegment = ""
      isSegmentNegated = false
    }
...
private final class PatternCompiler(private val pattern: String, private var flags: Int) {
  ...
  private def compileCharacterClass(): String = {
    ...
    /* If there is a leading '^', the first segment is negated.
     *
     * After the first original segment, any '^' is just a normal
     * character (which will be escaped in the compiled alternative).
     *
     * `isSegmentNegated` will be processed in `finishSegment()`.
     */
    if (pIndex != len && pattern.charAt(pIndex) == '^') {
      isSegmentNegated = true
      pIndex += 1
    }
    ...
    while (pIndex != len) {
      ...
      val startCodePoint: Int = (dispatchCP: @switch) match {
        case ']' =>
          ...
          finishSegment()
          val finalResult =
            if (thisConjunct == "") impossiblePattern
            else "(?:" + conjunction + thisConjunct + ")" // TODO? Avoid the grouping if possible
          return finalResult
