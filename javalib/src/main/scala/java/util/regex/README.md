...
The compiler is in the file `PatternCompiler`, and is invoked at the time of `Pattern.compile()`.
...
There are however a few features that either never supported, or only supported when targeting a recent enough version of ECMAScript.
...
* `\p{javaMethodName}` character classes,
* the `\G` boundary matcher, *except* if it appears at the very beginning of the regex (e.g., `\Gfoo`),
* embedded flag expressions with inner group, i.e., constructs of the form `(?idmsuxU-idmsuxU:X)`,
* embedded flag expressions without inner group, i.e., constructs of the form `(?idmsuxU-idmsuxU)`, *except* if they appear at the very beginning of the regex (e.g., `(?i)abc` is accepted, but `ab(?i)c` is not), and
...
* the internal format of `\p{name}` character classes,
...
Since `UNICODE_CASE` is only supported in ES 2015+, this implies that 'u' is supported, and hence that we leave all the handling of case insensitivity to the native RegExp.
...
`Pattern` adds the 'g' flag for its general-purpose instance of `RegExp` (the one used for everything except `Matcher.matches()`).
...
This is why to maintain `groupNumberMap`.
...
*A*, *B*, and *C* stand for any nested character class content, while respecting the precedence of the operators, including potentially empty strings.
...
* `[^ABC]` is equivalent to `(?![B])[^AC]`
* `[A\cB]` is equivalent to `[ACB]` where `C` is the expansion of the predefined character class `\c`, *unless* it is of the form `^...`
* `[^A\cB]` is equivalent to `[^ACB]` where `C` is the expansion of the predefined character class `\c`, *unless* it is of the form `^...`
* `[\c]` is equivalent to `\c`

Note that the `^` has a special meaning only right after a `[`, and only until the next `[`, `]` or `&&`.
For example, in `[^ab[c]&&d]`, the `^` applies to `ab`, but not to `c` nor `d`.

Using those rewrite rules, we can always reduce a complex character class so that it does not contain any union, intersection, negation or predefined character class.

For example,

* `[^a\dc]` expands to `[^a0-9c]` in ASCII mode, or `[^a\p{Nd}c]` in Unicode mode
* `[a-z&&[^m-p]]` expands to `(?=[a-z])[^m-p]`
* `[^2\D5]` expands to `[^2\P{Nd}5]` in Unicode mode, or `(?![\D])[^25]` in ASCII mode, which then gives `(?!\D)[^25]` and finally `(?![^0-9])[^25])`
...
To address that, we have special a posteriori handling in `Matcher.find()`.

#### Concretely

A single code point that is a high surrogate `x` is compiled to `(?:x(?![L]))`, where `L` is `\uDC00-\uDFFF`, the range of all low surrogates.
The negative look-behind group prevents a match from separating the high surrogate from a following low surrogate.
...
The only one that does not is the call `codePointNotAmong(thisSegment)` inside `CharacterClassState.finishSegment()`.
