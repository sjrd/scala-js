...
private[java] object ScalaOps {
  ...
  final class SimpleRange(start: Int, end: Int) {
    @inline
    def foreach[U](f: js.Function1[Int, U]): Unit = {
      var i = start
      while (i < end) {
        f(i)
        i += 1
  ...
  class JavaIterableOps[A] private[ScalaOps] (
    ...
    @inline def foreach[U](f: js.Function1[A, U]): Unit =
      __self.iterator().scalaOps.foreach(f)

    @inline def count(f: js.Function1[A, Boolean]): Int =
      __self.iterator().scalaOps.count(f)

    @inline def exists(f: js.Function1[A, Boolean]): Boolean =
      __self.iterator().scalaOps.exists(f)

    @inline def forall(f: js.Function1[A, Boolean]): Boolean =
      __self.iterator().scalaOps.forall(f)

    @inline def indexWhere(f: js.Function1[A, Boolean]): Int =
      __self.iterator().scalaOps.indexWhere(f)

    @inline def findFold[B](f: js.Function1[A, Boolean])(
        default: js.Function0[B])(g: js.Function1[A, B]): B = {
      __self.iterator().scalaOps.findFold(f)(default)(g)
    }

    @inline def foldLeft[B](z: B)(f: js.Function2[B, A, B]): B =
      __self.iterator().scalaOps.foldLeft(z)(f)

    @inline def reduceLeft[B >: A](f: js.Function2[B, A, B]): B =
