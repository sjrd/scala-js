...
object Character {
  ...
  @inline def isEmoji(codePoint: Int): scala.Boolean =
    UnicodeData.isEmoji(codePoint)

  @inline def isEmojiPresentation(codePoint: Int): scala.Boolean =
    UnicodeData.isEmojiPresentation(codePoint)

  @inline def isEmojiModifier(codePoint: Int): scala.Boolean = {
    /* So far, there is exactly one range of 5 emoji modifiers.
     * See https://www.unicode.org/Public/17.0.0/ucd/emoji/emoji-data.txt
     * and https://www.unicode.org/reports/tr51/tr51-29.html#Diversity
     *
     *   1F3FB..1F3FF ; Emoji_Modifier # light skin tone..dark skin tone
     */
    codePoint >= 0x1f3fb && codePoint <= 0x1f3ff
  }

  @inline def isEmojiModifierBase(codePoint: Int): scala.Boolean =
    UnicodeData.isEmojiModifierBase(codePoint)

  @inline def isEmojiComponent(codePoint: Int): scala.Boolean =
    UnicodeData.isEmojiComponent(codePoint)

  @inline def isExtendedPictographic(codePoint: Int): scala.Boolean =
    UnicodeData.isExtendedPictographic(codePoint)
