...
object IntegerLong {
   ...
   *
   * The overflowBarrier must be ops.divideUnsigned(ops.minInt, radix).
   ...
   * `ops.minInt = n * radix`. In that case, we will fail to detect the
   * overflow if `result == (n - 1) * radix` just before the multiplication.
   * After the multiplication, it will be `ops.minInt`, which is out of bounds.
  ...
  def parseSignedImpl[I, F](s: String, radix: Int, overflowBarrier: I)(
    ...
    while (i != len) {
      ...
      if (digit < zero || unsigned_>(result.value, overflowBarrier))
        fail()
      result.value = result.value * fromUnsignedInt32(radix) + digit
     ...
     */
    if (unsigned_>(result.value, maxInt - sign))
      fail()
  ...
  def parseUnsignedImpl[I, F](s: String, radix: Int, overflowBarrier: I)(
    ...
    while (i != len) {
       ...
       */
      if (unsigned_<(result.value, digit))
        fail()
      i += 1
    }
  ...
  def compress[I, F](i: I, mask: I)(implicit ops: IntFloatBits[I, F]): I = {
    ...
    while (j < logBitSize) {
      val mp = parallelSuffix(mk.value)
      val mv = mp & m.value // bits to move
      m.value = (m.value ^ mv) | (mv >>> (1 << j)) // compress m
      val t = x.value & mv
      x.value = (x.value ^ t) | (t >>> (1 << j)) // compress x
      mk.value = mk.value & ~mp
      j += 1
    }
  ...
  def expand[I, F](i: I, mask: I)(implicit ops: IntFloatBits[I, F]): I = {
    ...
    while (j >= 0) {
       ...
       */
      x.value = ((x.value ^ t) & mv) ^ x.value

      j -= 1
    }

    x.value & mask // clear out extraneous bits
  }
