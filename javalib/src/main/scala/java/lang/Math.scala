...
object Math {
  ...
  def scalb(d: scala.Double, scaleFactor: scala.Int): scala.Double = {
    ...
    val subnormalExpAjustment = -minEForSubnormalResult + 1 // adjust range so that minE becomes 1
    val twoPowMinusAjustment = 1.0 / (1L << subnormalExpAjustment).toDouble // exact division
     ...
     * and maxE). Therefore, at this newE is in the range of valid exponents if
     * and only if `scalb` does not overflow nor underflow.
     */
    ...
    if (isNormalExponent(newE)) {
      ...
      makeResult(newE + subnormalExpAjustment, signAndMantissa) * twoPowMinusAjustment
    } else {
  ...
  def scalb(f: scala.Float, scaleFactor: scala.Int): scala.Float = {
    ...
    val minEForSubnormalResult = -mbits - 1 // 1 additional bit for rounding up to MinPositiveValue
    val subnormalExpAjustment = -minEForSubnormalResult + 1 // adjust range so that minE becomes 1
    val twoPowMinusAjustment = 1.0f / (1 << subnormalExpAjustment).toFloat // exact division
