...
object Math {
  ...
  def scalbGeneric[I, F](x: F, scaleFactor: scala.Int)(implicit ops: IntFloatBits[I, F]): F = {
    ...
    val twoPowMinusAjustment = fone / intToFloat(one << subnormalExpAjustment) // exact division
    ...
    if (!isNormalExponent(e)) {
      ...
      signAndMantissa.value = (bits & signBit) | ((bits << (clz - (bitSize - mbits - 1))) & mmask)
    }
    ...
    if (isNormalExponent(newE)) {
      ...
      makeResult(newE, signAndMantissa.value)
      ...
      makeResult(newE + subnormalExpAjustment, signAndMantissa.value) * twoPowMinusAjustment
    } else {
