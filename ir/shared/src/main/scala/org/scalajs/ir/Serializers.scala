    ...
    def writeTree(tree: Tree): Unit = {
      ...
      tree match {
        ...
        case NullTypedClosure(tpe) =>
          writeTagAndPos(TagNullTypedClosure)
          writeType(tpe)
  ...
  private final class Deserializer(buf: ByteBuffer) {
    ...
    private def readTreeFromTag(tag: Byte): Tree = {
      ...
      (tag: @switch) match {
        ...
        case TagNullTypedClosure => NullTypedClosure(readType().asInstanceOf[ClosureType])
      ...
      val newMethods = for (method <- methods) yield {
        if (method.methodName.simpleName.nameString != "newInstance") {
          ...
          val newBody = if (secondParam.ptpe == IntType) {
             ...
             *   componentType2 = getClass(newArray(componentType2, 0))
             *   i = i + 1
             * }
            ...
            Block(
              ...
              While(BinaryOp(BinaryOp.Int_!=, i.ref, ArrayLength(secondParam.ref)), {
                Block(
                  Assign(
                    componentType2.ref,
                    GetClass(BinaryOp(BinaryOp.Class_newArray, componentType2.ref, IntLiteral(0)))
                  ),
                  Assign(
                    i.ref,
                    BinaryOp(BinaryOp.Int_+, i.ref, IntLiteral(1))
