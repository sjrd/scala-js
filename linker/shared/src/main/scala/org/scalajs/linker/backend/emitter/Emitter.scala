...
final class Emitter(config: Emitter.Config, prePrinter: Emitter.PrePrinter) {
  ...
  private def genClass(linkedClass: LinkedClass,
    ...
    for (methodDef <- linkedMethods) {
      ...
      if (emitAsStaticLike) {
        ...
        main ++= extractWithGlobalsAndChanged(methodCache.getOrElseUpdateWithChanged(methodDef.version, {
    ...
    if (linkedClass.hasInstances && kind.isAnyNonNativeClass) {
      ...
      val ctorWithGlobals = extractChanged {
        ...
        if (isJSClass) {
          ...
          ctorCache.getOrElseUpdateWithChanged(ctorVersion,
      ...
      val memberMethodsWithGlobals = for {
        ...
        extractChanged(methodCache.getOrElseUpdateWithChanged(version,
      ...
      val exportedMembersWithGlobals = for {
        ...
        extractChanged(memberCache.getOrElseUpdateWithChanged(version,
  ...
  private final class ClassCache
      ...
      with VersionedValueCache[DesugaredClassCache] {
  ...
  private final class MethodCache[T]
      extends knowledgeGuardian.KnowledgeAccessor
      with VersionedValueCache[WithGlobals[T]] {

    def getOrElseUpdateWithChanged(version: Version,
