...
private[emitter] final class KnowledgeGuardian(config: Emitter.Config) {
  ...
  private def computeInterfaceTypeTestInfos(
    ...
    var lastIDWithBucket = InterfaceTypeTestInfo.LastReservedID
    ...
    for (clazz <- sortedClasses) {
      ...
      val id = (className, bucket) match {
        case (CloneableClass, _) =>
          InterfaceTypeTestInfo.CloneableID
        case (SerializableClass, _) =>
          InterfaceTypeTestInfo.SerializableID
        case (_, InterfaceTypeTestInfo.NoBucket) =>
