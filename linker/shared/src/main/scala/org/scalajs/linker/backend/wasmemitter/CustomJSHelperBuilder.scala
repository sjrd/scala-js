  ...
  def genJSNativeLoadSpec(jsNativeLoadSpec: JSNativeLoadSpec): js.Tree = {
    def genFollowPath(owner: js.Tree, path: List[String]): js.Tree = {
      path.foldLeft(owner) { (owner, item) =>
        genJSBracketSelect(owner, js.StringLiteral(item))
      }
    }
    ...
    jsNativeLoadSpec match {
      ...
      case JSNativeLoadSpec.Import(module, path) =>
        genFollowPath(js.VarRef(js.Ident("imported" + ctx.jsNameGen.genModuleName(module))), path)
...
object CustomJSHelperBuilder {
  ...
  def genJSBracketSelect(qual: js.Tree, item: js.Tree)(
      implicit pos: Position): js.Tree = {
    // !!! Almost duplicate with emitter.JSGen.genBracketSelect
    item match {
      case js.StringLiteral(name) if js.Ident.isValidJSIdentifierName(name) && name != "eval" =>
