...
import CustomJSHelperBuilder.genJSBracketSelect
...
private class FunctionEmitter private (
    ...
    lhs match {
      ...
      case JSSelect(qualifier, item) =>
        genThroughCustomJSHelper(List(qualifier, item, rhs), NoType) { allJSArgs =>
          ...
          js.Assign(genJSBracketSelect(jsQualifier, jsItem), jsRhs)
        }
  ...
  private def genJSSelect(tree: JSSelect): Type = {
    ...
    genThroughCustomJSHelper(List(qualifier, item)) { allJSArgs =>
      ...
      js.Return(genJSBracketSelect(jsQualifier, jsItem))
    }
  }
  ...
  private def genJSFunctionApply(tree: JSFunctionApply): Type = {
    ...
    genThroughCustomJSHelper(fun :: args) { allJSArgs =>
      ...
      // !!! Similar code in `emitter.FunctionEmitter`: protect the `jsFun`
      val jsProtectedFun = jsFun match {
        case _:js.DotSelect | _:js.BracketSelect | js.VarRef(js.Ident("eval", _)) =>
          js.Block(js.IntLiteral(0), jsFun)
        case _ =>
          jsFun
      }
      js.Return(js.Apply(jsProtectedFun, jsArgs))
    }
  }
  ...
  private def genJSMethodApply(tree: JSMethodApply): Type = {
    ...
    genThroughCustomJSHelper(receiver :: method :: args) { allJSArgs =>
      ...
      js.Return(js.Apply(genJSBracketSelect(jsReceiver, jsMethod), jsArgs))
    }
  }
