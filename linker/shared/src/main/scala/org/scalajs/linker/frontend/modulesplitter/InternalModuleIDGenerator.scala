...
private[modulesplitter] final class InternalModuleIDGenerator(
  ...
  private val avoidSet: Set[String] =
    SortedSet(publicModuleIDs.map(_.id).toSeq: _*)(CaseInsensitiveStringOrdering)
  ...
  def makeFromDigestGenerator(avoid: Iterable[ModuleID]): FromDigestGenerator = {
    ...
    val longestPrefixLike_internalDashes =
      if (avoid.isEmpty && publicModuleIDs.isEmpty) 0
      else (avoid.iterator ++ publicModuleIDs.iterator).map(longestPrefixOfIDLike_internalDashes(_)).max
    ...
    new FromDigestGenerator(BasePrefix + ("-" * requiredDashCount))
  }
}
