    ...
    private def defineSpecializedArrayClasses(): List[Tree] = {
      specializedArrayTypeRefs.flatMap { componentTypeRef =>
        ...
        val getAndSet = if (arrayIndexOutOfBounds != CheckedBehavior.Unchecked) {
          ...
          val getName = genArrayClassPropertyWithOrigName(ArrayClassProperty.get)
          val setName = genArrayClassPropertyWithOrigName(ArrayClassProperty.set)
        ...
        val copyTo = if (esVersion >= ESVersion.ES2015) {
          ...
          val copyToName = genArrayClassPropertyWithOrigName(ArrayClassProperty.copyTo)
        ...
        val clone = MethodDef(static = false, genMethodIdent(cloneMethodName), Nil, None, {
      ...
      val initClass = {
        ...
        MethodDef(static = false, Ident(cpn.initClass), params, None, {
          Block(
              ...
              genEmptyMutableLet(internalName.ident),
              ForIn(genEmptyImmutableLet(prop.ident), ancestors, Block(
                internalName := prop,
                Break()
              )),
              if (hasParentData)
