...
private[optimizer] abstract class OptimizerCore(
  ...
  private def tryInsertAtFirstEvalContext(valName: LocalName, valTree: Tree, body: Tree): Option[Tree] = {
    ...
    def recs(bodies: List[Tree]): EvalContextInsertion[List[Tree]] = bodies match {
      ...
      case firstBody :: restBodies =>
        rec(firstBody) match {
          ...
          case PureSoFar             => recs(restBodies).mapOrKeepGoing(firstBody :: _)
          case Failed                => Failed
        }
    }
    ...
    def rec(body: Tree): EvalContextInsertion[Tree] = {
      ...
      body match {
        ...
        case LoadModule(className) =>
          if (hasElidableConstructors(className)) PureSoFar
          else Failed
        ...
        case Apply(flags, receiver, method, args) =>
          rec(receiver) match {
            ...
            case PureSoFar if isNotNull(receiver) =>
        ...
        case BinaryOp(op, lhs, rhs) =>
          ...
          rec(lhs) match {
            ...
            case PureSoFar =>
              rec(rhs) match {
                case Success(newRhs) => Success(BinaryOp(op, lhs, newRhs))
                case Failed          => Failed

                case PureSoFar =>
                  (op: @switch) match {
                    case Int_/ | Int_% | Long_/ | Long_% | String_+ =>
                      Failed
                    case String_charAt if semantics.stringIndexOutOfBounds != CheckedBehavior.Unchecked =>
                      Failed
                    case _ =>
        ...
        case ArrayLength(array) =>
          rec(array).mapOrKeepGoingIf(ArrayLength(_))(
              keepGoingIf = nullPointersAreUnchecked || isNotNull(array))

        case ArraySelect(array, index) =>
          rec(array) match {
            case Success(newArray) =>
              Success(ArraySelect(newArray, index)(body.tpe))
            case PureSoFar if isNotNull(array) =>
        ...
        case RecordSelect(record, field) =>
          rec(record).mapOrFailed(RecordSelect(_, field)(body.tpe))

        case IsInstanceOf(expr, testType) =>
          rec(expr).mapOrKeepGoing(IsInstanceOf(_, testType))

        case AsInstanceOf(expr, tpe) =>
          rec(expr).mapOrKeepGoingIf(AsInstanceOf(_, tpe))(
              keepGoingIf = semantics.asInstanceOfs == CheckedBehavior.Unchecked)

        case GetClass(expr) =>
          rec(expr).mapOrKeepGoingIf(GetClass(_))(
              keepGoingIf = nullPointersAreUnchecked || isNotNull(expr))
    ...
    rec(body) match {
      ...
      case PureSoFar =>
        // the val was never actually used
  ...
  private def nullPointersAreUnchecked: Boolean =
  ...
  private def checkNotNull(texpr: PreTransform)(implicit pos: Position): PreTransform = {
    ...
    if (!tpe.isNullable || nullPointersAreUnchecked) {
      texpr
    } else {
  ...
  private def checkNotNull(expr: Tree)(implicit pos: Position): Tree = {
    if (nullPointersAreUnchecked || isNotNull(expr))
      expr
    else
  ...
  private def checkNotNullStatement(texpr: PreTransform)(implicit pos: Position): Tree = {
    if (!texpr.tpe.isNullable || nullPointersAreUnchecked)
...
private[optimizer] object OptimizerCore {
  ...
  private sealed abstract class EvalContextInsertion[+A] {
    ...
    def mapOrKeepGoing[B](f: A => B): EvalContextInsertion[B] = this match {
      ...
      case _          => this.asInstanceOf[EvalContextInsertion[B]]
    }

    def mapOrKeepGoingIf[B](f: A => B)(keepGoingIf: => Boolean): EvalContextInsertion[B] = this match {
      case Success(a) => Success(f(a))
      case PureSoFar  => if (keepGoingIf) PureSoFar else Failed
      case Failed     => Failed
    }
  ...
  private object EvalContextInsertion {
    ...
    case object PureSoFar extends EvalContextInsertion[Nothing]
  }
