...
private[optimizer] abstract class OptimizerCore(
  ...
  private def finishTransformStat(stat: PreTransform): Tree = stat match {
    ...
    case PreTransIf(cond, thenp, elsep, tpe) =>
      ...
      (newThenp, newElsep) match {
        ...
        case (Skip(), _) =>
          val newCond = finishTransformExpr(foldNot(cond))
          If(newCond, newElsep, newThenp)(VoidType)
        case _ =>
  ...
  private def foldIf(cond: PreTransform, thenp: PreTransform, elsep: PreTransform)(
    ...
    def isCombinable(preTrans: PreTransform): Boolean = preTrans match {
   ...
   *
   *  All the operands must be pure, and have a meaningful `==` test.
   */
