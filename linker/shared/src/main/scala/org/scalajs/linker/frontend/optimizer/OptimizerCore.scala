...
private[optimizer] abstract class OptimizerCore(
  ...
  private def expandLongOps(pretrans: PreTransform)(cont: PreTransCont)(
    ...
    pretrans match {
      ...
      case PreTransBinaryOp(op, lhs, PreTransLit(IntLiteral(r))) if isIntDivOp(op) && (r == 0 || isUnsignedPowerOf2(r)) =>
        val optimizedBody = integerDivisions.makeOptimizedDivision(op, r)
        System.err.println("-----------------------------\n" + r + "\n" + optimizedBody.show + "\n\n")
        expandOptimizedDivision(lhs, optimizedBody)

      case PreTransBinaryOp(op, lhs, PreTransLit(LongLiteral(r))) if isLongDivOp(op) && (r == 0L || isUnsignedPowerOf2(r)) =>
        val optimizedBody = integerDivisions.makeOptimizedDivision(op, r)
        expandOptimizedDivision(lhs, optimizedBody)
