  ...
  private def constrainedLub(lhs: Type, rhs: Type, upperBound: Type, isStat: Boolean): Type = {
    ...
    if (isStat || upperBound == VoidType) {
      ...
      (lhs, rhs) match {
        ...
        case (ClassType(lcls, lnullable, lexact), ClassType(rcls, rnullable, rexact)) if lcls == rcls =>
          ClassType(lcls, lnullable || rnullable, lexact && rexact)
        case (ArrayType(ltypeRef, lnullable, lexact), ArrayType(rtypeRef, rnullable, rexact)) if ltypeRef == rtypeRef =>
