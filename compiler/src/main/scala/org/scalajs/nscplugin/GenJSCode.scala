  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
    ...
    private def genRegisterReflectiveInstantiationForModuleClass(sym: Symbol)(
      ...
      val loadModuleFunArg = js.NewLambda(
          js.NewLambda.Descriptor(encodeClassName(AbstractFunctionClass(0)), Nil,
              MethodName("apply", Nil, jstpe.ClassRef(ir.Names.ObjectClass)),
              Nil, jstpe.AnyType),
          js.TypedClosure(Nil, Nil, jstpe.AnyType, genLoadModule(sym), Nil)
    ...
    private def genRegisterReflectiveInstantiationForNormalClass(sym: Symbol)(
      ...
      if (ctors.isEmpty) {
        ...
        val objectArrayRef = jstpe.ArrayTypeRef(objectClassRef, 1)
        ...
        val classClassRef = jstpe.ClassRef(ir.Names.ClassClass)
        val classArrayRef = jstpe.ArrayTypeRef(classClassRef, 1)

        val tuple2Ctor = MethodName.constructor(List(objectClassRef, objectClassRef))

        val newInstanceFunDescriptor = {
          js.NewLambda.Descriptor(encodeClassName(AbstractFunctionClass(1)), Nil,
              MethodName("apply", List(objectClassRef), objectClassRef),
        ...
        val constructorsInfos = for {
          ...
          val newInstanceClosure = {
             ...
             * }
             */
            js.TypedClosure(Nil, argsParamDef :: Nil, jstpe.AnyType, {
    ...
    private def genAnonFunction(originalFunction: Function): js.Tree = {
      ...
      val formalCaptureTypeRefs = captureSyms.toList.map(sym => toTypeRef(sym.info))
      ...
      val (allFormalCaptureTypeRefs, allFormalCaptures, body, allActualCaptures) = if (!compileAsStaticMethod(target)) {
        val thisActualCapture = genExpr(receiver)
        val thisFormalCaptureTypeRef = toTypeRef(receiver.tpe)
      ...
      if (isFunctionSymbol(funSym)) {
        ...
        /* This is an LMF-capable SAM type. Use a `NewLambda` if we do not need
         * any bridges; otherwise synthesize a SAM wrapper class.
         */
        ...
        if (samBridges.isEmpty) {
          ...
          /* We need bridges; synthesize a SAM wrapper.
          ...
          val samWrapperClassName = synthesizeSAMWrapper(funSym, sam, samBridges, closure, ctorName)
          js.New(samWrapperClassName, js.MethodIdent(ctorName), allActualCaptures)
        }
      }
    }
    ...
    private def synthesizeSAMWrapper(funSym: Symbol, samInfo: SAMFunction,
        samBridges: List[Symbol], closure: js.TypedClosure,
